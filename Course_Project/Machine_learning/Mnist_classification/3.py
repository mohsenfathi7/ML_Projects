...
def gradient(data, label, hidden_layer_output, p, v, num_input_features, num_hidden_units, num_output_class):
    ...
    x2 = np.ones(num_hidden_units).reshape(num_hidden_units, 1)
...
u, v, bias, loss, test_error, train_error = train(train_data, labels, test_data, test_labels, 100, 100, 0.5, 784, 60, 60000)
print("min error on train:", np.amin(train_error))
print("min error on test:", np.amin(test_error))
np.save("loss_1_2_linear.npy", loss)
np.save("train_error_1_2_linear.npy", train_error)
np.save("test_error_1_2_linear.npy", test_error)
...
def init_params(num_input_features, num_hidden_units, num_output_class):
    ...
    bias = np.random.uniform(-0.5, 0.5, size=num_hidden_units).reshape(num_hidden_units, 1)
...
def train(data, label, test_data, test_label, batch_size, epoch, learning_rate, num_input_features, num_hidden_units, data_size):
    ...
    train_error = np.zeros(data_size * epoch)
    test_error = np.zeros(data_size * epoch)
    loss = np.zeros(data_size * epoch)

    for i in range(epoch):
        for j in range(data_size):
            h, p = forward(data[j], u, v, bias, num_hidden_units)
            loss[data_size * i + j] = cost(p[label[j]])
            v_delta, u_delta, bias_delta = gradient(data[j], label[j], h, p, v, num_input_features,num_hidden_units, 10)
            u, v, bias = update_params(u, v, bias, u_delta, v_delta, bias_delta, learning_rate)
            train_error[data_size * i + j] = error(data, label, u, v, bias, 60000, num_hidden_units)
            test_error[data_size * i + j] = error(test_data, test_label, u, v, bias, 10000, num_hidden_units)
            print(j)
            # earning_rate = learning_rate * 0.1
        permutation = np.random.permutation(data_size)
        print(i)
        data = [data[k] for k in permutation]
        print(i)
        label = [label[k] for k in permutation]

    return u, v, bias, loss, test_error, train_error


train_data = feature_standardization(images, 60000)
test_data = feature_standardization(test_images, 10000)

u, v, bias, loss, test_error, train_error = train(train_data, labels, test_data, test_labels, 1, 20, .5, 784, 60, 60000)
print("min error on train:", np.amin(train_error))
print("min error on test:", np.amin(test_error))
np.save("SGD_loss_1_1_60.npy", loss)
np.save("SGD_train_error_1_1_60.npy", train_error)
np.save("SGD_test_error_1_1_60.npy", test_error)